...
# `mlp_pareto_verification` WandB project (mlp / cifar10 study, still
# running - `fetch_grouped_runs`/`build_runs_summary` below tolerate missing
# or not-yet-existing folds by warning and skipping rather than failing, so
# this can be re-run at any point while the study is still in progress).
#
...
MODEL_NAME = "mlp"  # for compute_quantization_bits' reference backbone
OUTPUT_PDF = "mlp_pareto_cv_report.pdf"
N_FOLDS = 5
...
# design choice - see module docstring above). Note: with the study still
# running, this front only reflects configs/folds completed so far.
df["is_pareto_optimal"] = compute_pareto_mask(
    df, x="compression_rate", y="cv_val_acc_mean"
)
n_opt = int(df["is_pareto_optimal"].sum())
print(f"Loaded {len(df)} CV-verified configs from {ENTITY}/{PROJECT}")
print(f"  {n_opt} Pareto-optimal, {len(df) - n_opt} dominated (post-CV, so far)")
print(
    df[
        [
            "trial_number",
...
# %%
# The one "fundamental" plot: CV-verified Validation Accuracy vs. compression
...
# internally by build_compression_breakdown. With the study still running,
# this may legitimately be empty (no MoMos-family config on the front yet) -
# skip the figure rather than crash.
breakdown_df = build_compression_breakdown(df, label_backbone)
if breakdown_df.empty:
    print(
        "No Pareto-optimal MoMos-family configs found - skipping compression breakdown plot."
    )
else:
...
# (resolve these from the WandB `mlp_pareto_verification` project for the
...
_DETAIL_SPECS = [
    {"method": "hierarchical_momos2d", "trial_number": 136},  # V-Fold MoMs
    {"method": "qat", "trial_number": 9},
    {"method": "momos2d", "trial_number": 133},
    {"method": "momos2d", "trial_number": 124},
]
...
# Some names may not exist yet (study still running) - fetch_grouped_runs warns
# and drops those rather than failing.
groups = {
